...
async def fetch_joinable_groups_autocomplete(search: str, guild_id: int, user_id: int, admin: bool = False):
    async with await get_connection() as conn:
        async with conn.cursor() as cursor:
            ...
            LEFT JOIN (SELECT id, group_id FROM group_relations WHERE user_id = %s) AS r \
            ON g.id = r.group_id WHERE g.name LIKE %s AND r.id IS NULL AND (g.private = FALSE OR %s = TRUE)", (guild_id, user_id, f"{search}%", admin, ))
